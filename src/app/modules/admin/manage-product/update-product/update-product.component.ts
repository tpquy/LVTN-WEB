...
export class UpdateProductComponent implements OnInit {
  ...
  constructor(
    ...
    this.laptopID = data.id;
  }

  ngOnInit(): void {
    this.initAllTag(this.objList);
    this.getCPUList('');
    this.getDetailCategoryTag(this.laptopID);
  }
  ...
  async onConfirmAdd() {
    ...
    if (!this.updateForm.invalid && checkLanguageId && checkContent && regExpContent && regExpSymbol && !this.duplicateCode && checkSymbol && checkCharsContent && checkCharsSymbol && checkFile) {
  ...
  putLaptop(requestBody) {
    this.laptopService.putLaptop(this.laptopID, requestBody).subscribe(data => {
      const result = {
  ...
  getDetailCategoryTag(id) {
    this.laptopService.getDetailLaptop(id).subscribe(data => {
      this.response = data;
      this.selectedBrand = data.brand;
      this.selectedCPU = data.cpu;
      this.selectedRam = data.ram;
      this.selectedRom = data.rom;
      this.selectedScreen = data.screen;
      this.selectedgraphicsCard = data.graphicsCard;
  ...
  setViewData() {
    this.updateForm = new FormGroup({
      // brand: new FormControl({ value: this.response.brand.name.name, disabled: true }),
      ...
      code: new FormControl({ value: this.response.code, disabled: true }),
      cpu: new FormControl('' + this.response.cpu.id),
      ram: new FormControl(''  + this.response.ram.id),
      rom: new FormControl('' + this.response.rom.id),
      screen: new FormControl('' + this.response.screen.id),
      graphicsCard: new FormControl('' + this.response.graphicsCard.id),
      material: new FormControl('' + this.response.material.id),
      os: new FormControl('' + this.response.os.id),
      type: new FormControl('' + this.response.type.id),
      weight: new FormControl('' + this.response.weight),
      releaseYear: new FormControl(''+ this.response.releaseYear),
      price: new FormControl(''+ this.response.price),
      quantity: new FormControl('' + this.response.quantity),
      // attachment: new FormControl('')
    });
    this.attachFilePreview = this.response.img;
  }
  ...
  onSelectFileAttach(event, doneClass) {
    console.log("Da goi toi onselectFIle");
